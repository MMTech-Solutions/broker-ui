...
export async function POST(request: Request): Promise<Response> {
  const auth = await resolveBrokerAuthCredentials("admin");
  if (!auth) {
  ...
  try {
    upstream = await fetch(upstreamUrl, {
      ...
      body,
      cache: "no-store",
      // Cross-origin redirects may discard Authorization. This endpoint must
      // already be the final HTTPS destination.
      redirect: "manual",
    });
  } catch {
    return Response.json(
