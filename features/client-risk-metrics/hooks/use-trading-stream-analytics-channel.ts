...
export function useTradingStreamAnalyticsChannel({
  ...
  onPhaseMetrics: (payload: RiskMetricChangedPayload) => void;
  onResync: () => void;
  ...
  const onPhaseMetricsRef = useRef(onPhaseMetrics);
  ...
  useEffect(() => {
    onPhaseMetricsRef.current = onPhaseMetrics;
  }, [onPhaseMetrics]);
  ...
  useEffect(() => {
    ...
    const subscribe = async (welcome: WelcomeFrame) => {
      ...
      try {
        for (const stream of ["analytics", "phase_metrics"] as const) {
    ...
    const connect = () => {
      ...
      socket.onmessage = (event) => {
        ...
        if (frame.account_id !== accountId) return;

        if (frame.type === "account.analytics.dashboard" && isRecord(frame.data)) {
          onDashboardRef.current(frame.data.data);
          return;
        }

        if (frame.type === "account.analytics.updated" && isRecord(frame.data)) {
          onUpdateRef.current(frame.data);
          return;
        }

        if (frame.type === "account.analytics.resync") {
          onResyncRef.current();
          return;
        }

        if (frame.type === "account.phase_metrics.updated" && frame.data) {
          onPhaseMetricsRef.current(frame.data as RiskMetricChangedPayload);
          return;
        }
        ...
        if (
          ...
          (frame.stream === "analytics" || frame.stream === "phase_metrics")
        ) {
    ...
    // Deferring the first dial lets that probe cleanup cancel the disposable
    // connection before it reaches the WS handshake and subscribe POST.
